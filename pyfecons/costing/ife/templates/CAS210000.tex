...
\begin{itemize}
    \item \textbf{Nuclear Island:} Fusion Heat Island (22.01.01-06), Hot Cell (22.04), Fuel Storage (22.05)
    \item \textbf{Power Conversion:} Turbine (23.xx), Heat Exchanger (22.02-03)
    \item \textbf{Electrical:} Power Supply (22.01.07), Cryogenics (22.01.03 cooling)
    \item \textbf{Support:} Reactor Auxiliaries (22.06), Control Room (22.07)
    \item \textbf{New:} Isotope Separation (22.01.12), Target Factory (22.01.08), Direct Energy Converter (22.01.09), Assembly Hall (22.01.11)
\end{itemize}
...
\hline
\multicolumn{11}{l}{\textbf{Electrical \& Power Systems}} \\
...
21.15.00	&	Cryogenics	&	Steel frame	&	2.0	&	11.0	&	33.0	&	5.5	&	2003	&	2019	&	1.19	&	0.7	\\
\hline
\multicolumn{11}{l}{\textbf{Support Buildings}} \\
...
\hline
\multicolumn{11}{l}{\textbf{Administrative \& Site}} \\
...
\hline
\multicolumn{11}{l}{\textbf{New Buildings (CAS 22 Subsystem-Derived)}} \\
21.18.00	&	Isotope Separation Plant	&	Industrial	&	126.1	&	40.0	&	80.0	&	15	&	48000	&	2024	&	1.00	&	150.0	\\
21.18.01	&	Target Factory (IFE)	&	Clean Room	&	42.0	&	30.0	&	60.0	&	12	&	21600	&	2024	&	1.00	&	50.0	\\
21.18.02	&	Direct Energy Conv. Bldg	&	Concrete \& Steel	&	21.0	&	20.0	&	40.0	&	10	&	8000	&	2024	&	1.00	&	25.0	\\
21.18.03	&	Assembly Hall	&	High Bay Steel	&	12.6	&	60.0	&	40.0	&	30	&	72000	&	2024	&	1.00	&	15.0	\\
\hline
21.98.00	&	Spare parts allowance	&		&		&		&		&		&		&		&		&	0.0	\\
21.99.00	&	Contingency allowance	&		&	0.0	&		&		&		&		&		&		&	C211900	\\
\hline
21.00.00	&	\textbf{Structures and site facs}	&		&		&		&		&		&		&		&		&	\textbf{C210000}	\\
    \end{tabular}
    \end{tiny}
    \caption{Cost categories for buildings, and their cost estimation. Buildings are organized by functional area and mapped to the CAS 22 equipment subsystems they house. Note: IFE includes Target Factory (21.18.01) for pellet fabrication.}
    \label{tab:buildings}
\end{table}
...
\begin{itemize}
\item Cost Category 21.04 - Heat Rejection Building. This account covers the costs associated with the systems needed for rejecting heat from the power plant, scaled to the rejected thermal power. Houses CAS 22.02 (main coolant) and CAS 22.03 (auxiliary cooling) equipment. Cost is \$ C210400 M using NETL Reference case B12A, account 14.2, boiler building bare erected costs for a 686MW gross power plant in 2019 \$, apply escalation per table \ref{tab:buildings}.
\item Cost Category 21.05 - Electrical Equipment \& Power Systems Building. It includes the costs for all electrical equipment and power systems, scaling with the net electrical power output of the plant. Houses CAS 22.01.07 (power supplies for driver lasers). Cost is \$ C210500 M, using a smaller version of the turbine building - multiple floors, concrete, per table \ref{tab:buildings}.
\item Cost Category 21.06 - Plant Auxiliary Systems Building. This account addresses the costs of auxiliary systems in the plant, such as compressed air, inert gas storage, and distribution systems, scaling with the gross electrical power. Houses CAS 22.06 (other reactor plant equipment). Cost is \$ C210600 M as smaller version of the turbine building - multiple floors, concrete, per table \ref{tab:buildings}.
\item Cost Category 21.07 - Hot Cell Building. The cost for the Hot Cell Building, designed for handling large and hazardous components, is scaled in relation to the Reactor Building. Houses CAS 22.04 (radioactive waste processing) and remote handling equipment. Cost is \$ C210700 M as a smaller version of the Heat Island Building by 50\%, per table \ref{tab:buildings}.
...
\item Cost Category 21.10 - Fuel Handling and Storage Building. This account covers the costs for buildings related to handling and storing fuel, scaled according to tritium usage or fusion power. Houses CAS 22.05 (fuel handling and storage) equipment. Cost is \$ C211000 M, scaled relative to the administration building per table \ref{tab:buildings}.
\item Cost Category 21.11 - Control Room Building. It encompasses the expenses for the control room building, essential for plant operations, with costs similar to those in previous plant designs. Houses CAS 22.07 (instrumentation and control) systems. Cost is \$ C211100 M scaled relative to the administration building per table \ref{tab:buildings}.
...
\item Cost Category 21.15 - Cryogenic and Inert Gas Storage Building. It includes the costs for buildings that store cryogenic and inert gases, essential for plant operations. For IFE, this primarily supports cryogenic target handling rather than superconducting magnets, resulting in reduced cost (0.3x scaling). Cost is \$ C211500 M, per table \ref{tab:buildings}.
...
\subsection*{New Buildings (Derived from CAS 22 Subsystems)}

The following buildings were added to provide dedicated facilities for equipment subsystems identified in CAS 22 that require separate structures:

\begin{itemize}
    \item Cost Category 21.18 - Isotope Separation Plant. This industrial facility houses CAS 22.01.12 (isotope separation systems) for fuel preparation. For D-T fuel, this includes heavy water (D$_2$O) extraction via Girdler-Sulfide process and lithium-6 enrichment via COLEX process. For D-D fuel, only deuterium extraction is required. For p-B$^{11}$ fuel, protium purification and boron-11 enrichment are needed. Cost is \$ C211800 M, scaled from CANDU heavy water plant costs (\$100-500M for 1GW capacity) with fuel-type adjustment factors: D-T (1.0x), D-D (0.6x), D-He$^3$ (0.8x), p-B$^{11}$ (1.2x).

    \item Cost Category 21.18.01 - Target Factory (IFE Only). This clean room facility houses CAS 22.01.08 (target factory) equipment for pellet fabrication and cryogenic target handling. Specific to Inertial Fusion Energy (IFE) reactor designs, this building provides the controlled environment necessary for manufacturing fusion fuel pellets at the required precision and production rate. Scaled from NIF target production facility estimates. Cost is \$ C211801 M.

    \item Cost Category 21.18.02 - Direct Energy Converter Building. This separate structure houses CAS 22.01.09 (direct energy conversion) equipment when not integrated into the main reactor building. More significant for aneutronic fuels (D-He$^3$, p-B$^{11}$) with higher charged particle fractions. Cost is \$ C211802 M, scaled by fuel type: D-T (0.2x), D-D (0.4x), D-He$^3$ (1.0x), p-B$^{11}$ (1.5x).

    \item Cost Category 21.18.03 - Laser/Component Assembly Hall. This high-bay facility houses CAS 22.01.11 (installation) activities, providing space for large component assembly before installation into the reactor. For IFE, this includes laser system assembly and alignment. Scaled from NIF and ITER assembly hall estimates. Cost is \$ C211803 M.
\end{itemize}
